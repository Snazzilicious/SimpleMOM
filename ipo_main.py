...
GeneralUtils.stdout("Beginning IPO")
...
# construct impedance matrix - should be consistent with MOM formulation
...
for i in range(n_faces):
	...
	A = 2 * areas[vis_map[i]] # the '2' comes from 2n\times H
...
casename = args.output if args.output is not None else "ipo_{0:s}_{1:.0f}Hz".format( Path(mesh_filename).stem, freq )
...
while err > current_tol and it < max_its :
	sols.append( b - M.dot( sols[-1] ) )
	err = np.linalg.norm( sols[-1] - sols[-2] ) / len(sol0)
	it += 1
